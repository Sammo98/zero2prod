// tokio::test is the testing equivalent same as tokio::main
// also spares you from having to specify #[test] attribute
...
async fn spawn_app() -> TestApp {
    // Spawns a temporary application just for unit tests.

    // Run initialisation of logger only once
    Lazy::force(&TRACING);
    // Bind to a random port (:0) and create address from that port
    let listener = TcpListener::bind("127.0.0.1:0")
        .expect("Failed to bind random port");
    let port = listener.local_addr().unwrap().port();
    let address = format!("http://127.0.0.1:{}", port);

    // Get configuration from configuration.yaml - for creating connection string
    let mut configuration = get_configuration().expect("Failed to read configuration.");

    // Change database name from the "prod" database to a random database name
    configuration.database.database_name = Uuid::new_v4().to_string();

    // Configure the database (follow function)
    let connection_pool = configure_database(&configuration.database).await;

    // Create the server with run function (follow)
    let server = run(listener, connection_pool.clone()).expect("Failed to bind address");

    // Spawn the server and return the test application data
...
pub async fn health_check_works(){
    ...
    let response = client
        ...
        .send()
        .await
        .expect("Failed to execute ressponse");
